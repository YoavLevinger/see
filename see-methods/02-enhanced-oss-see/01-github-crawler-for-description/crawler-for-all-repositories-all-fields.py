...
def fetch_repo_metadata(owner, repo):
    ...
    if response.status_code == 200:
        data = response.json()
        return data.get("created_at"), data.get("size", 0), data.get("description", "N/A")
    return None, 0, ""
    return None, 0


def fetch_developer_metrics(owner, repo, created_at):
    ...
    try:
        ...
        if commit_response.status_code == 200:
            ...
            # fallback to estimate commit count using contributors
            if contrib_response.status_code == 200:
                commit_count = sum([contributor.get('contributions', 0) for contributor in developers])
            if len(commits) >= 2:
        ...
        if release_response.status_code == 200:
            ...
            if releases:
                first_release_date = datetime.strptime(releases[0]['created_at'], "%Y-%m-%dT%H:%M:%SZ")

        if created_at and first_release_date:
            created_dt = datetime.strptime(created_at, "%Y-%m-%dT%H:%M:%SZ")
            delta = (first_release_date - created_dt).days
...
def process_repositories(cursor, repositories, limit=None):
    ...
    for i in range(0, len(repositories), batch_size):
        ...
        for owner, repo in batch:
            ...
            dev_count, days_active, effort, effort_months, sloc_add, sloc_del, sloc_mod, days_to_release, first_commit, first_release, commit_count = fetch_developer_metrics(
