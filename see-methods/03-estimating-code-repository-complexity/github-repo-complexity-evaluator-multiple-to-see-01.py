...
# computes a normalized complexity score, estimates development effort,
# prints everything in a table, and exports results to CSV and dashboard.
...
def analyze_code_metrics(repo_path):
    ...
    for file in analysis:
        if not file.function_list:
            continue  # skip files with no functions parsed
...
# Estimate development effort in hours based on complexity score
def estimate_effort(complexity_score, base_effort=50, scaling_factor=950):
    return round(base_effort + complexity_score * scaling_factor, 2)
...
def evaluate_repo_complexity(owner, repo_name):
    ...
    effort_estimate = estimate_effort(complexity_score)

    all_metrics["owner"] = owner
    all_metrics["repo"] = repo_name
    all_metrics["complexity_score"] = complexity_score
    all_metrics["estimated_effort_hours"] = effort_estimate

    # Print metrics in a clean table
    print("\nComplexity formula: C = 0.3·(LOC_norm) + 0.4·(CC_norm) + 0.3·(HV_norm)")
    print("Effort estimation formula: Effort = 50 + C × 950 (in hours)")
    table_data = [[k, v] for k, v in all_metrics.items()]
    print(tabulate(table_data, headers=["Metric", "Value"], tablefmt="github"))

    shutil.rmtree(temp_dir)
    return all_metrics

# Batch evaluation of multiple repositories with logging output
